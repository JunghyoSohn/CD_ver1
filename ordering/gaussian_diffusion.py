...
try:
    ...
    from functorch import vmap, jacrev  # noqa: F401


## gaussian diffusion
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps, scale = 0.01, beta_start = 0.0001, beta_end = 0.02):
    """
    ...
    if schedule_name == "linear":
        ...
        return np.linspace(
            beta_start_scaled, beta_end_scaled, num_diffusion_timesteps, dtype=np.float64
        )
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, self._scale_timesteps(t), **model_kwargs) # (x[0],model_output[0])
        jacobian = vmap(jacrev(model),randomness="same")(x.unsqueeze(1),self._scale_timesteps(t).unsqueeze(1))
        # x.unsqueeze(1)
        # model_t = lambda x : model(x, self._scale_timesteps(t), **model_kwargs)
        #from functorch import vmap, jacrev, make_functional
        #jacobian = vmap(jacrev(model_t))(x)
        #jacobian = vmap(jacrev(model))(x.unqueeze(1),self._scale_timesteps(t).unqueeze(1))
        #jac = torch.autograd.functional.jacobian(model,(x, self._scale_timesteps(t)), vectorize=True)
        #grads = torch.autograd.grad(model_output, x, grad_outputs = torch.ones(2*(model_output.shape[-1],)), is_grads_batched = True)
        #grads = torch.autograd.grad(model_output, x, grad_outputs = torch.ones(model_output.shape[-1]), is_grads_batched = True)
        #grads = torch.autograd.grad(model_output, x, grad_outputs = torch.ones_like(model_output), is_grads_batched = True)
        #grads = torch.autograd.grad(model_out_1, x[:1], grad_outputs = torch.ones_like(model_out_1), is_grads_batched = True)
    ...
    def ddim_sample(
        ...
        return result_dict # debug with torch.cat(list(result_dict.values()),axis = 1)[0] or (out["pred_xstart"][0],x[0])
    ...
    def ddim_sample_loop(
        ...
        return (final[-1][final_output].detach(), final) if progress else (final[final_output], [final])
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, mean_loss=True):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if "vb" in terms:
                terms["loss"] = mean_flat(terms["mse"]) + terms["vb"]
            else:
                terms["loss"] = mean_flat(terms["mse"]) if mean_loss else terms["mse"]
        else:
...
class ScheduleSampler(ABC):
    ...
    def sample(self, batch_size, device):
        ...
        #indices = th.from_numpy(indices_np / len(p)).long().to(device)
